...
void dynarray_special_resize(struct dynarray* da, int start) {
  /*
   * Allocate space for the new array.
   */
  void** new_data = malloc(da->capacity*2 * sizeof(void*));
  assert(new_data);
   ...
   */
  for (int i = 0; i < da->size; i++) {
    new_data[i] = da->data[(i+start)%da->capacity];
  }
...
void dynarray_queue_insert(struct dynarray* da, void* val) {
  assert(da);
   ...
   */
  if (da->size == da->capacity) {
    dynarray_special_resize(da, da->start);
    da->data[da->size] = val;
    da->start = 0;
  } else {
